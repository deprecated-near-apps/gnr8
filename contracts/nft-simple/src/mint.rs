...
impl Contract {
    ...
    pub fn nft_mint(
        ...
        if is_lazy_mint.is_some() {
            assert!(series.approved_account_ids.contains(&owner_id), "predecessor_account_id (market?) not approved to lazy mint series");
        }
        ...
        assert!(num_tokens < series.params.max_supply.0, "Cannot mint anymore of series: {}", series_name);
        ...
        if let Some(perpetual_royalties) = perpetual_royalties {
            ...
            assert!(royalty.len() < 8, "Cannot have more than 8 royalty receivers");
        ...
        let required_storage_in_bytes = self.extra_storage_in_bytes_per_token + new_token_size_in_bytes;
